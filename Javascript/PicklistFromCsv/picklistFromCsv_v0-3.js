...
{
	var arrayCsv = csvEntry.split(csvDelimiter);
	return arrayCsv;
...
function tickFormularyFromHtmlCsv(htmlTagId, className)
{
	...
	var arrayCsvValues = csvToArray(document.getElementById(htmlTagId).innerHTML, csvDelimiter);
...
function csvFromCheckboxValues(className, checkedCondition, delimiter)
{
	...
	for(var i=0; i<arrayOfChoises.length; i++)
	{
		...
		{
			csvString += arrayOfChoises[i]+delimiter;
		}
	}
 ...
 * arrayValuesTick = csvToArray(csvList); tickCheckboxesByValue(arrayValuesTick,
 * vecIds);
 *  }
 */
...
//var arrayProd = csvToArray("prod1, prod2, prod3");
